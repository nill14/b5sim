...
object AttackProcessor extends App with LogHelper {
  
  def loadFragments: Iterator[Array[String]] = {
	  val is = this.getClass.getResourceAsStream("/attackDetail.txt")
...
case class AttackItem(row: Array[String]) {
  val shipName = row(0)
  val owner = row(1)
  val exp = row(2).toDouble
  val status = parseStatus(row(3))
  val ship = getShip(row(4))
  ...
  def toFleetClass = {
    ...
    FleetClass(Some(ship.shipClass), Some(ship), str, SpaceShip.label(ship), 
        count, fullHp, us, ul, status, exp, price, 0)
  }
