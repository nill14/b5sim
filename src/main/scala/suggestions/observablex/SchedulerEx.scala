...
object SchedulerEx {
  ...
  private val poolExecutor = Executors.newFixedThreadPool(4)
  val poolScheduler: Scheduler = Schedulers.executor(poolExecutor)
  
//  val poolScheduler: Scheduler = ExecutorScheduler(Executors.newFixedThreadPool(4))
//  
//  val swingScheduler: Scheduler = ExecutorScheduler(new Executor() {
